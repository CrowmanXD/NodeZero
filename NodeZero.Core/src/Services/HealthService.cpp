...
HealthService::HealthService()
    : m_MaxHealth(GameConfig::HEALTH_DEFAULT),
      ...
      m_RegenRate(0.0f),
	m_HealthDepletionRate(BASE_DEPLETION_RATE), //Refactor: Now uses constant
      m_HealthDepletionInterval(DEPLETION_INTERVAL), //Refactor: Now uses constant
      m_HealthTimer(0.0f),
      m_CurrentLevel(1) {
}
...
void HealthService::ApplyDepletion(float deltaTime) {
    ...
    if (m_HealthTimer >= m_HealthDepletionInterval) {
        // Refactored: Uses constant for level scaling
        const float scalingMultiplier = 1.0f + (m_CurrentLevel - 1) * LEVEL_SCALING_FACTOR;
        const float scaledDepletionRate = m_HealthDepletionRate * scalingMultiplier;
